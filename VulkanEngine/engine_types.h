...
class IResource {
    friend class Engine;
public:
    ENGINE_API virtual ~IResource() {};
    ENGINE_API virtual void destroy(void*) = 0;
    ENGINE_API virtual ResourceType getType() = 0;
...
struct EventTimer
{
    float triggerTime;               // absolute time when it fires
    std::function<void()> callback;  // what to run
};
...
struct UniformBufferObject {
	glm::mat4 model; // Slot 1
	glm::mat4 view;  // Slot 2
	glm::mat4 proj;  // Slot 3
};
...
class PhysicsEventListener : public PxSimulationEventCallback {
public:
...
struct SceneGameObject {
    ...
    std::string gameObjectType; // custom class here
